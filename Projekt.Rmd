...
#join with r_vdem
colnames(r_vdem)[1] <- "cntbrthc_long"
dat2 <- dat2 %>% 
  left_join(r_vdem, by = "cntbrthc_long")

dat_wrong_cntbrthc_long <- dat2 %>% 
  filter(FALSE == is.na(cntbrthc_long) & TRUE == is.na(year))
...
```{r}
# Non-political trust
h2_nonpolitical <- lm(non_political_trust_index ~ migrant_gen * cntbrthc + agea + eduyrs + hinctnta,
                      data = dat2)
